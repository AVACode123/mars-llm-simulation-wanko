...
class Simulation:
    ...
    def initialize_agents(self):
        ...
        crew = [
            ("Aki", "Commander（指揮官）"),
            ("Ryo", "Pilot（パイロット）"),
            ("Ken", "Mechanical Engineer（機械エンジニア）"),
            ("Mina", "Life Scientist（生命科学者）"),
            ("Sora", "Geologist（地質学者）"),
            ("Taro", "EVA Specialist（船外活動スペシャリスト）"),
            ("Yui", "Communications Officer（通信担当）"),
            ("Hana", "Medical Doctor（医師）"),
            ("Pochi", "Mission Dog（ペットの犬）"),
        ]
    ...
    def step_simulation(self):
        ...
        # Phase 4: Execute movement by declared target_place
        outside_position = (0, -self.half_space_size)
        outside_position = (0, -self.half_space_size)
        valid_places = {place["name"] for place in self.places}

        for agent, action_decision in action_decisions:
            target = action_decision.get("target_place")

            if target in valid_places:
                agent.go_to_place(target)

            elif target == "outside":
                agent.position = outside_position
                agent.total_moves += 1

            elif action_decision.get("action") == "move" and action_decision.get("direction"):
                agent.move(action_decision["direction"])

            else:
                # target_place が不明な場合は動かさない
                logger.warning(
                    f"Agent {agent.id}: invalid target_place={target}. Staying at current position."
                )
        ...
        # Update stress after movement
        for agent, action_decision in action_decisions:
            agent.update_stress(action_decision)
        ...
        for agent in self.agents:
            self.agent_logs.append({
                ...
                "stress": getattr(agent, "stress", None)  # ←まだ無い場合でもOK
            })
    ...
    def run(self):
        ...
        if self.output_dir:
            ...
            df = pd.DataFrame(self.agent_logs)

            # 最新版（上書き）
            latest_path = os.path.join(self.output_dir, "agent_logs_latest.csv")
            df.to_csv(latest_path, index=False, encoding="utf-8-sig")


            # タイムスタンプ版（履歴）
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            history_path = os.path.join(self.output_dir, f"agent_logs_{timestamp}.csv")
        ...
        if self.output_dir:
            os.makedirs(self.output_dir, exist_ok=True)
            df = pd.DataFrame(self.agent_logs)
            df.to_csv(os.path.join(self.output_dir, "agent_logs.csv"), index=False, encoding="utf-8-sig")
